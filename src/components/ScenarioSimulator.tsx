...
const scenarioSteps: Record<number, Step[]> = {
  ...
  3: [
    {
      id: 1,
      title: 'Введение в ХАССП',
      description: 'Система ХАССП (HACCP) — это система анализа рисков и критических контрольных точек для обеспечения безопасности пищевой продукции.',
      type: 'info',
      points: 10,
      icon: 'Info',
      learningMaterial: {
        title: 'Основы системы ХАССП',
        content: [
          'ХАССП — международный стандарт управления безопасностью пищевых продуктов (Hazard Analysis and Critical Control Points).',
          'Система обязательна для всех предприятий пищевой промышленности согласно ТР ТС 021/2011.',
          '7 принципов ХАССП: анализ опасностей, определение ККТ, установление критических пределов, мониторинг, корректирующие действия, верификация, документирование.',
          'Внедрение ХАССП снижает риски выпуска небезопасной продукции на 95% и предотвращает штрафы до 1 млн рублей.'
        ]
      }
    },
    {
      ...
      description: 'Первый шаг — выявление всех потенциальных биологических, химических и физических опасностей в производстве.',
      type: 'quiz',
      question: 'Какой опасный фактор относится к биологическим угрозам?',
      options: [
        { id: 'a', text: 'Остатки моющих средств в продукте', correct: false },
        { id: 'b', text: 'Металлические частицы от оборудования', correct: false },
        { id: 'c', text: 'Сальмонелла в сыром мясе', correct: true },
        { id: 'd', text: 'Превышение температуры хранения', correct: false }
      ],
      correctAnswer: 'c',
      points: 20,
      icon: 'AlertTriangle',
      learningMaterial: {
        title: 'Типы опасных факторов',
        content: [
          'Биологические: бактерии (сальмонелла, листерия, E.coli), вирусы, паразиты, плесень, дрожжи.',
          'Химические: пестициды, антибиотики, тяжёлые металлы, моющие средства, токсины.',
          'Физические: стекло, металл, камни, пластик, древесина, волосы.',
          'Для каждого этапа производства составляется таблица всех возможных опасностей с оценкой вероятности и тяжести последствий.'
        ]
      }
    },
    {
      id: 3,
      title: 'Определение критических контрольных точек (ККТ)',
      description: 'ККТ — это этапы производства, где можно предотвратить, устранить или снизить опасность до приемлемого уровня.',
      type: 'quiz',
      question: 'Какой этап производства молочной продукции является ККТ?',
      options: [
        { id: 'a', text: 'Упаковка готового продукта', correct: false },
        { id: 'b', text: 'Пастеризация молока при 72°C в течение 15 секунд', correct: true },
        { id: 'c', text: 'Маркировка упаковки датой производства', correct: false },
        { id: 'd', text: 'Транспортировка в магазины', correct: false }
      ],
      correctAnswer: 'b',
      points: 25,
      icon: 'Target',
      learningMaterial: {
        title: 'Критические контрольные точки (ККТ)',
        content: [
          'ККТ — этапы, где утрата контроля приведёт к недопустимому риску для здоровья потребителя.',
          'Примеры ККТ: термообработка (пастеризация, стерилизация), металлодетекция, охлаждение, заморозка.',
          'Для определения ККТ используется "дерево принятия решений" из стандарта ГОСТ Р 51705.1-2001.',
          'Каждая ККТ должна иметь критические пределы (температура, время, pH), которые нельзя превышать.'
        ]
      }
    },
    {
      id: 4,
      title: 'Мониторинг и измерения',
      description: 'Постоянный контроль параметров в ККТ для подтверждения, что процесс находится под управлением.',
      type: 'action',
      points: 15,
      icon: 'Activity',
      learningMaterial: {
        title: 'Система мониторинга ККТ',
        content: [
          'Мониторинг проводится в реальном времени: автоматические датчики температуры, давления, pH.',
          'Частота измерений зависит от критичности: для пастеризации — непрерывно, для хранения — каждые 4 часа.',
          'Все результаты фиксируются в журналах ККТ с указанием даты, времени, значения, ФИО ответственного.',
          'При отклонении от критических пределов автоматически включается сигнализация и останавливается процесс.'
        ]
      }
    },
    {
      ...
      description: 'Процесс вышел за критические пределы. Примите решение о судьбе партии продукции.',
      type: 'quiz',
      question: 'Температура пастеризации молока упала до 68°C (норма 72°C). Что делать?',
      options: [
        { id: 'a', text: 'Продолжить процесс, отклонение незначительное', correct: false },
        { id: 'b', text: 'Остановить линию, изолировать партию, провести повторную пастеризацию', correct: true },
        { id: 'c', text: 'Отправить продукцию на склад с пометкой "для переработки"', correct: false },
        { id: 'd', text: 'Увеличить время пастеризации на 5 секунд', correct: false }
      ],
      correctAnswer: 'b',
      points: 25,
      icon: 'AlertCircle',
      learningMaterial: {
        title: 'Корректирующие действия при отклонениях',
        content: [
          'Немедленная остановка процесса при выходе за критические пределы — обязательное требование.',
          'Партия продукции изолируется и маркируется как "несоответствующая" до выяснения обстоятельств.',
          'Проводится расследование причин: неисправность оборудования, человеческий фактор, сбой программы.',
          'Решение о судьбе партии: утилизация, переработка, повторная обработка — принимает специалист по качеству.'
        ]
      }
    },
    {
      id: 6,
      title: 'Верификация системы ХАССП',
      description: 'Периодическая проверка эффективности системы ХАССП через аудиты и лабораторные испытания.',
      type: 'quiz',
      question: 'Как часто проводится внутренний аудит системы ХАССП?',
      options: [
        { id: 'a', text: 'Ежедневно', correct: false },
        { id: 'b', text: 'Еженедельно', correct: false },
        { id: 'c', text: 'Не реже 1 раза в год', correct: true },
        { id: 'd', text: 'Только при выявлении несоответствий', correct: false }
      ],
      correctAnswer: 'c',
      points: 20,
      icon: 'CheckCircle2',
      learningMaterial: {
        title: 'Верификация и валидация ХАССП',
        content: [
          'Внутренний аудит системы ХАССП проводится минимум 1 раз в год согласно ТР ТС 021/2011.',
          'Внешняя сертификация (ISO 22000, FSSC 22000) требуется для работы с международными партнёрами.',
          'Лабораторные испытания готовой продукции на микробиологию проводятся ежемесячно.',
          'Пересмотр плана ХАССП обязателен при изменении рецептуры, технологии, оборудования или поставщиков сырья.'
        ]
      }
    },
    {
      id: 7,
      title: 'Документация ХАССП',
      description: 'Оформите документацию по результатам внедрения системы ХАССП на предприятии.',
      type: 'action',
      points: 35,
      icon: 'FileText',
      learningMaterial: {
        title: 'Обязательная документация ХАССП',
        content: [
          'План ХАССП: описание продукта, блок-схема производства, анализ опасностей, таблица ККТ с критическими пределами.',
          'Журналы мониторинга ККТ: записи всех измерений с подписями ответственных лиц.',
          'Протоколы корректирующих действий: описание инцидента, причины, меры, результаты проверки.',
          'Отчёты по верификации: результаты внутренних аудитов, лабораторных анализов, калибровки приборов.'
        ]
      }
    }
  ]
